...
export const metadata = buildPageMetadata({
  ...
  description: "把需求分析、候选生成、机器校验、人工裁决、测试执行、回归和报告编排成可恢复、可审计的 AI 辅助测试工作流。",
  path: "/knowledge/ai-testing-workflow-orchestration",
  tags: ["AI辅助测试", "工作流编排", "测试用例生成", "人工审核", "质量门禁"],
});
...
export default function AiTestingAssetProductionPage() {
  return <div className="mx-auto max-w-5xl animate-fade-in">
    ...
    <KnowledgeLayout sections={sections} searchPlaceholder="搜索工作流、用例生成、机器检查、人工审核和失败恢复...">
      <Header />
      ...
      <S id="evidence" n="02" t="先证明输入完整，再启动工作流" b="文档是证据">
      ...
      <S id="pipeline" n="03" t="把测试工作拆成可验收、可恢复的节点" b="一步一产物">
      ...
      <S id="roles" n="04" t="让 AI、脚本和人工各自做擅长的判断" b="人机分工先于自动化">
        <Table title="工作流职责矩阵" headers={["角色", "适合负责", "不能单独决定"]} rows={[
          ["AI", "材料归纳、候选生成、相似项聚类和证据摘要", "业务争议、高风险授权和正式发布"],
          ["确定性脚本", "Schema、字段、覆盖映射、重复、敏感信息和分布检查", "复杂业务语义和风险接受"],
          ["测试工程师", "风险判断、业务裁决、抽样审核和发布建议", "绕过审批直接修改生产数据"],
          ["业务负责人", "需求歧义、资金权限规则和风险接受", "替代技术证据判断执行结果"],
        ]} />
        <Flow title="一次完整交付" items={[["AI候选", "提取与生成"], ["脚本门禁", "结构与规则"], ["人工裁决", "业务与风险"], ["自动执行", "安全沙箱"], ["结果回流", "缺陷与资产"]]} />
      </S>

      <S id="contract" n="05" t="节点之间用固定产物交接" b="脚本和人都能消费">
  ...
  "questions": []
      ...
      <S id="checks" n="06" t="机器 Check 只处理确定性问题" b="快筛，不替业务裁决">
      ...
      <S id="human" n="07" t="人工把候选结果变成正式资产" b="按证据裁决">
      ...
      <S id="versions" n="08" t="输入、产物、执行和审核都按版本隔离" b="禁止覆盖历史">
      ...
      <S id="regression" n="09" t="工作流任一节点变化都要回放固定评估集" b="局部变化也会影响全链路">
        ...
        <Code title="workflow-regression.json">{`{
  ...
  "compare": ["rule_recall", "human_acceptance", "p95", "token_cost"]
      ...
      <S id="recovery" n="10" t="流程中断后从最近合格产物继续" b="恢复而不是重写">
      ...
      <S id="practice" n="11" t="用一个小版本跑通完整测试工作流" b="练习与检查">
        <Card title="练习"><List ordered items={[
          "选择一个含 3～5 条规则变更的脱敏需求，完成文档元素对账。",
          "选用已经验证的 Prompt、Skill 和输出 Schema，并登记版本。",
...
function Header() { return <header className="mb-10"><div className="mb-3 font-mono text-[10px] uppercase tracking-[0.22em] text-neon-cyan">Phase 05 / AI-Native QA Engineering 03</div><h1 className="mb-3 bg-gradient-to-r from-neon-cyan to-neon-purple bg-clip-text text-3xl font-bold text-transparent md:text-4xl">AI 测试工作流编排与人机协作教程</h1><p className="mb-6 text-lg leading-8 text-text-secondary">把需求分析、候选生成、机器检查、人工裁决、执行、回归和报告连接成一条可以恢复、审计和持续改进的测试工作流。</p><div className="flex flex-wrap gap-6 text-sm text-text-secondary"><span>11 个章节</span><span>Workflow + Human Gate</span><span>机器门禁 + 人工审核</span></div></header>; }
