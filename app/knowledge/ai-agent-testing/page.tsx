...
export const metadata = buildPageMetadata({
  ...
  description: "以电商售后 Agent 为贯穿案例，测试任务规划、工具选择与参数、多轮状态、重试幂等、终止条件、人工审批和安全回退。",
  path: "/knowledge/ai-agent-testing",
  tags: ["AI Agent测试", "工具调用", "任务轨迹", "多轮状态", "人机协作"],
});
...
export default function AgentTestingPage() {
  return <div className="mx-auto max-w-5xl animate-fade-in">
    ...
    <KnowledgeLayout sections={sections} searchPlaceholder="搜索 Agent、工具轨迹、状态、审批与回退...">
      <Header />
      <S id="scope" n="01" t="把目标、工具和副作用放在同一条轨迹中测试" b="贯穿案例">
        <Card title="案例：电商售后 Agent"><p>用户要求查询订单、判断退款资格并创建售后申请。Agent 可以调用订单查询、政策检索和创建工单工具；读取可以自动执行，真实写入必须展示对象、参数和影响，并获得一次性人工确认。</p></Card>
        <Flow title="Agent 执行链" items={[["用户目标", "任务与授权"], ["任务计划", "步骤与依赖"], ["工具决策", "选择与参数"], ["状态变化", "执行与回读"], ["任务终态", "完成、失败或转人工"]]} />
      ...
      <S id="tools" n="03" t="分别测试选没选、选哪个、参数对不对" b="三层都要正确">
  ...
  return "ALLOW";
      ...
      <S id="state" n="04" t="逐轮验证状态和完整轨迹" b="答案之外还有状态">
...
assert isolated_by(session_id, actor_id, tenant_id)`}</Code>
      ...
      <S id="planning" n="05" t="主动构造规划失败" b="计划要能停、能改">
      ...
      <S id="retry" n="06" t="重试必须受预算、错误类型和幂等约束" b="失败不能变重复操作">
  ...
  on_exhausted: "ESCALATE"
      ...
      <S id="fallback" n="07" t="降级时减少自主性，而不是降低安全标准" b="失败时少做事">
      ...
      <S id="human" n="08" t="在人机边界前冻结副作用" b="确认不是装饰">
  ...
  single_use: true
      ...
      <S id="evaluation" n="09" t="结合硬断言、轨迹评分和人工复核" b="机器判边界，人判业务">
  ...
  "human_decision":"ACCEPT"
      ...
      <S id="release" n="10" t="用硬门禁交付可控 Agent" b="发布前演练失败">
...
function Header() { return <header className="mb-10"><div className="mb-3 font-mono text-[10px] uppercase tracking-[0.22em] text-neon-cyan">Phase 03 / Agent Quality 01</div><h1 className="mb-3 bg-gradient-to-r from-neon-cyan to-neon-purple bg-clip-text text-3xl font-bold text-transparent md:text-4xl">AI Agent 测试实战教程</h1><p className="mb-6 text-lg leading-8 text-text-secondary">验证 Agent 怎样理解目标、选择工具、维护状态并控制副作用，让每一步都能解释、能停止、能回退。</p><div className="flex flex-wrap gap-6 text-sm text-text-secondary"><span>10 个章节</span><span>规划 + 工具轨迹</span><span>审批、幂等与安全回退</span></div></header>; }
