...
\`\`\`mermaid
flowchart LR
  A[历史需求文档] --> F[向量化索引<br/>RAG]
  B[历史测试用例] --> F
  C[历史 Bug 单] --> F
  D[代码仓库] --> F
  E[数据库 Schema<br/>定时任务配置] --> F
\`\`\`
...
\`\`\`mermaid
flowchart TD
  A[新需求描述] --> B[意图识别<br/>判断涉及模块]
  B --> C[从知识库检索相关上下文]
  C --> D[历史用例<br/>代码变更历史<br/>历史 Bug<br/>数据库表结构]
  D --> E[注入 Top-K 相关片段<br/>控制模型窗口]
  E --> F[AI 生成用例]
\`\`\`
...
\`\`\`mermaid
flowchart TD
  A[需求进来] --> B[AI 读取相关模块代码<br/>输出影响分析]
  B --> C[AI 生成实现方案<br/>2-3 个选项 + 权衡]
  C --> D[你选择方案]
  D --> E[AI 写代码<br/>限定文件范围]
  E --> F[自动跑测试<br/>Playwright + Vitest]
  F --> G[AI 输出改动报告<br/>修改文件 / 影响功能 / 风险点]
  G --> H[你审核并确认上线]
\`\`\`
